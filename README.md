...
| -------------------- | ---------------------------------- | ----------------------------------------------------------- |
| Next.js 16           | [next/](next/)                     | React framework with App Router, NextAuth.js, React Query   |
...
| Category    | Next.js     | Next-Supabase     | TanStack Start | Express (PG) | Express (Mongo) | NestJS          | FastAPI     | Spring Boot       | Spring Boot + Lombok |
| ----------- | ----------- | ----------------- | -------------- | ------------ | --------------- | --------------- | ----------- | ----------------- | -------------------- |
| Language    | TypeScript  | TypeScript        | TypeScript     | TypeScript   | TypeScript      | TypeScript      | Python      | Java 21           | Java 21              |
| Framework   | Next.js 16  | Next.js 15        | TanStack Start | Express      | Express         | NestJS          | FastAPI     | Spring Boot 3.5   | Spring Boot 3.5      |
| Database    | PostgreSQL  | Supabase PG       | PostgreSQL     | PostgreSQL   | MongoDB         | PostgreSQL      | PostgreSQL  | PostgreSQL        | PostgreSQL           |
| ORM         | Drizzle     | Supabase          | Drizzle        | Drizzle      | Mongoose        | TypeORM/Prisma  | SQLAlchemy  | Spring Data JPA   | Spring Data JPA      |
| Auth        | NextAuth.js | Supabase Auth     | Server fns     | JWT          | JWT + RBAC      | @nestjs/jwt     | python-jose | Spring Security   | Spring Security      |
| Cache       | Redis       | React Query       | Redis          | Redis        | Redis           | Redis           | Redis       | Redis             | Redis                |
| Validation  | Zod         | Zod               | Zod            | Zod          | Zod             | class-validator | Pydantic    | Jakarta Bean Val. | Jakarta Bean Val.    |
| Routing     | App Router  | App Router        | File-based     | Express      | Express         | Modules         | FastAPI     | Controllers       | Controllers          |
| Boilerplate | Manual      | Manual            | Minimal        | Manual       | Manual          | Opinionated     | Minimal     | Manual            | Lombok               |
| API Style   | REST        | REST              | REST           | REST         | REST            | REST            | REST        | REST              | REST                 |
| Extra       | -           | Real-time, Stripe | -              | -            | -               | -               | -           | -                 | -                    |
...
6. **TanStack Start** - Type-safe React framework with TanStack Router
7. **Spring Boot** - Enterprise Java (manual boilerplate)
8. **Spring Boot + Lombok** - Enterprise Java with Lombok (industry standard)
