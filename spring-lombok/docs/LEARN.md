...
Lombok auto-generates boilerplate code (getters, setters, constructors) at **compile time** via annotations.

### Why Use It?

| Benefit           | Impact                        |
| ----------------- | ----------------------------- |
| -60% boilerplate  | Cleaner, readable classes     |
| Less human error  | No typos in getters/setters   |
| Industry standard | Most Java/Spring shops use it |
...
### Example: Service Without Lombok
...
public class AuthService {
    private final UserRepository userRepository;
    private final JwtTokenProvider jwtTokenProvider;
    // Lombok generates constructor and logger!
}
```
...
```typescript
// TypeScript: Classes are naturally concise
class User {
  constructor(
    public id: number,
    public email: string,
...
// No need for manual getters/setters!

// Java without Lombok: 100+ lines
// Java with Lombok: 20 lines
```
...
| ------------------------------ | ---------------------------------------------- |
| Teaching Java fundamentals     | Write it out - learners should see boilerplate |
| Complex constructor validation | Explicit constructor with logic                |
| Circular references            | Use `@Getter(AccessLevel.NONE)`                |
| Debugging generated code       | IDE Lombok plugin shows synthetic methods      |

### In This Project

All entities (`User`, `RefreshToken`), DTOs, and services use Lombok:

- `@Data` + `@NoArgsConstructor` + `@AllArgsConstructor` on entities/DTOs
- `@Slf4j` + `@RequiredArgsConstructor` on services
- `@Getter` on exception classes
